          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            {breadcrumbs && (
              <nav className="breadcrumb mb-3" aria-label="Breadcrumb">
                <Link href="/" className="hover:text-white transition-colors text-navy-200">Home</Link>
                {breadcrumbs.map((crumb, i) => (
                  <span key={i} className="flex items-center gap-1.5">
                    <ChevronRight className="w-3 h-3 text-navy-400" />
                    {crumb.href && i < breadcrumbs.length - 1 ? (
                      <Link href={crumb.href} className="hover:text-white text-navy-200 transition-colors">{crumb.label}</Link>
                    ) : (
